...
public class AdvancedUtil {
	private static final Log logger = LogFactory.getLog(AdvancedUtil.class);
	...
	public static WeixinOauth2Token getOauth2AccessToken(String appId,
		...
		if (null != jsonObject) {
			try {
				...
				logger.error("获取网页授权凭证失败 errcode:{" + errorCode + "} errmsg:{"
						+ errorMsg + "}");
			}
		}
		return wat;
	...
	public static WeixinOauth2Token refreshOauth2AccessToken(String appId,
		...
		if (null != jsonObject) {
			try {
				...
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				logger.error("刷新网页授权凭证失败 errcode:{" + errorCode + "} errmsg:{"
						+ errorMsg + "}");
			}
		}
		return wat;
	...
	public static SNSUserInfo getSNSUserInfo(String accessToken, String openId) {
		...
		if (null != jsonObject) {
			try {
				...
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				logger.error("获取用户信息失败 errcode:{" + errorCode + "} errmsg:{"
						+ errorMsg + "}");
			}
		}
		return snsUserInfo;
	 ...
	 * @return boolean
	 */
	public static boolean validateOauth2AccessToken(String accessToken,
			String openId) {
		int errcode = 0;
		// 拼接请求地址
		String requestUrl = "https://api.weixin.qq.com/sns/userinfo?access_token=ACCESS_TOKEN&openid=OPENID";
